		...
		db.get().query(query1, (error, allrecipes, fields) => {
			...
			db.get().query(query2, (error, ingredients, fields) => {
				...
				db.get().query(query3, (error, units, fields) => {
					...
					res.render('myrecipes' , { title: 'My Recipes', user: req.session.username, id: 'myrecipes', categories: categories, units: units, ingredients: ingredients, allrecipes : allrecipes});
